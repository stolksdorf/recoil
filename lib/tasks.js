...
module.exports = function(gulp, configOverride){
	...
	gulp.task('config', require('./tasks/config').bind(this, config));
	...
	gulp.task('build', function (callback) {
		runSequence('clean', 'architecture', ['libs', 'assets', 'js', 'less', 'html', 'config'], callback);
	});
	gulp.task('update', function(callback){
		runSequence(['architecture', 'config', 'assets'], 'run', callback);
	});
	...
	gulp.task('cmds', function(){
		...
		console.log('    update  : Picks up new files and config changes, then runs the project');
